...
def sangue_oculto_first_graph():
    df = pd.read_csv("csv_files/sangue_oculto.csv", sep=';')
    ...
    fig.update_layout(
        title=dict(text="EXAME DE SANGUE OCULTO NAS FEZES"),
        plot_bgcolor='white',
        xaxis_title='Data',
...
def main():
    ...
    fig = px.line(df, x='Data', y='Total', color='Período', markers=True, title="Exames de sangue oculto Totais")
    fig.update_layout(plot_bgcolor='white')
    st.plotly_chart(fig)
    
    regioes = list(df.columns[4:10])
    for regiao in regioes:
        fig = px.line(df, x='Data', y=regiao, color='Período', markers=True, title=f"Exames de sangue oculto {regiao}")
        fig.update_layout(plot_bgcolor='white')
        st.plotly_chart(fig)
