...
def obitos_first_graph():
    df = pd.read_csv("csv_files/obitos.csv", sep=';')
    ...
    df = df[['Data', 'Ano', 'Mês', 'Período', '1 Região Norte', '2 Região Nordeste', '3 Região Sudeste', '4 Região Sul', '5 Região Centro-Oeste', 'Total',]]
...
def main():
    ...
    fig = px.line(df, x='Data', y='Total', color='Período', markers=True, title="Óbitos Totais")
    fig.update_layout(plot_bgcolor='white')
    st.plotly_chart(fig)
    
    regioes = list(df.columns[4:10])
    for regiao in regioes:
        fig = px.line(df, x='Data', y=regiao, color='Período', markers=True, title=f"Óbitos {regiao}")
        fig.update_layout(plot_bgcolor='white')
        st.plotly_chart(fig)
